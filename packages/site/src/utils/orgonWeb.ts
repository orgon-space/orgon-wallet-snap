 ...
 */
export function createOrgonWebService(
  fullHost?: string,
  chainId?: string,
): OrgonWebService {
 ...
 */
export function validateDeploymentParams(params: any): {
  isValid: boolean;
  error?: string;
} {
  ...
  if (
    params.callValue &&
    (typeof params.callValue !== 'number' || params.callValue < 0)
  ) {
    return {
      isValid: false,
      error: 'callValue должен быть неотрицательным числом',
    };
  }

  if (
    params.userFeePercentage &&
    (params.userFeePercentage < 0 || params.userFeePercentage > 30)
  ) {
    return {
      isValid: false,
      error: 'userFeePercentage должен быть от 0 до 30',
    };
  }

  if (
    params.feeLimit &&
    (typeof params.feeLimit !== 'number' || params.feeLimit <= 0)
  ) {
    return {
      isValid: false,
      error: 'feeLimit должен быть положительным числом',
    };
  }
